...
X = df[['HR_device', 'Number_of_friends', 'Average_BPM',
        'TeacherStudent_corr', 'Avg_student_corr', 'GC_teacher_to_student',
        'GC_student_to_teacher']]
...
numerical_features = ['HR_device', 'Number_of_friends', 'Average_BPM',
                      'TeacherStudent_corr', 'Avg_student_corr',
                      'GC_teacher_to_student', 'GC_student_to_teacher']
scaler = StandardScaler()
X_normalized = X.copy()
X_normalized[numerical_features] = scaler.fit_transform(X[numerical_features])

regr = linear_model.LinearRegression()

predicted = cross_val_predict(regr, X_normalized, y, cv=LeaveOneOut())
